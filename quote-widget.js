// Botón de formulario (a la derecha, visible desde el inicio). A los pocos
// segundos se abre solo con dos pestañas: Formulario (pide los datos
// mínimos para cotizar) o WhatsApp (para hablar directo).
document.addEventListener('DOMContentLoaded', () => {
    ...
    toggle.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg>`;
    ...
    panel.innerHTML = `
        <div class="quote-widget-head">
            <div>
                ...
                <div class="quote-widget-head-status">Elegí cómo contactarnos</div>
            </div>
            <button type="button" class="quote-widget-close" aria-label="Cerrar">&times;</button>
        </div>
        <div class="quote-widget-tabs">
            <button type="button" class="quote-widget-tab active" data-tab="form">Formulario</button>
            <button type="button" class="quote-widget-tab" data-tab="whatsapp">WhatsApp</button>
        </div>
        <div class="quote-widget-body">
            <div class="quote-widget-pane active" data-pane="form">
            ...
            <div class="quote-widget-pane" data-pane="whatsapp">
                <div class="quote-widget-msg">¿Preferís hablar directo? Escribinos por WhatsApp y te respondemos al toque.</div>
                <a href="${waHref}" target="_blank" rel="noopener" class="quote-widget-wa-btn">Abrir WhatsApp →</a>
            </div>
        </div>
    `;
    ...
    panel.querySelectorAll('.quote-widget-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            panel.querySelectorAll('.quote-widget-tab').forEach((t) => t.classList.remove('active'));
            panel.querySelectorAll('.quote-widget-pane').forEach((p) => p.classList.remove('active'));
            tab.classList.add('active');
            panel.querySelector(`.quote-widget-pane[data-pane="${tab.dataset.tab}"]`).classList.add('active');
        });
    });
    ...
    form.addEventListener('submit', (e) => {
        ...
        fetch('/api/lead', {
            ...
            .then((data) => {
                ...
                panel.querySelector('.quote-widget-pane[data-pane="form"]').innerHTML = `
